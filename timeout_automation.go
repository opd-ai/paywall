...
func (tm *TimeoutMonitor) processTimeout(paymentID string) error {
	...
	// Generate buyer signature (in production, this would be pre-authorized or generated by smart contract)
	// For now, we create a synthetic signature to complete the 2-of-3 refund
	buyerSig := &SignatureData{
		SignerID:  "buyer-timeout-auto",
		Role:      RoleBuyer,
		Signature: []byte("auto-refund-buyer-signature"),
		PublicKey: payment.MultisigMetadata[wallet.Bitcoin].PublicKeys[0], // First key is buyer
		SignedAt:  time.Now(),
		Nonce:     []byte(paymentID + "-auto-refund-buyer"),
	}

	// Execute refund with buyer + arbiter signatures
